...
const HouseForSaleDetails = () => {
    ...
    useEffect(() => {
        // Retrieve the selected listing from localStorage
        const listing = localStorage.getItem("selectedListing");
        if (listing) {
            setHouse(JSON.parse(listing));

            // Filter out the selected house and randomize others as similar houses
            const allListings = JSON.parse(localStorage.getItem("allListings")) || [];
            const similar = allListings
                .filter(item => item.id !== JSON.parse(listing).id)
                .sort(() => 0.5 - Math.random())
                .slice(0, 5); // get 5 random similar houses
            ...
            <div className="mt-8">
                ...
                <div className="grid sm:grid-cols-2 md:grid-cols-3 sm:gap-4 md:gap-6 px-6">
                    {similarHouses.map((similarHouse) => (
                        ...
                        >
                            {/* Custom Navigation Buttons */}
                            <div className={`absolute top-1/4 left-2 z-10 -translate-y-1/2 cursor-pointer swiper-button-prev-${similarHouse.id} bg-black/50 p-2 rounded-full text-white hover:bg-black/70`}>
                                <ChevronLeft size={20} />
                            </div>
                            <div className={`absolute top-1/4 right-2 z-10 -translate-y-1/2 cursor-pointer swiper-button-next-${similarHouse.id} bg-black/50 p-2 rounded-full text-white hover:bg-black/70`}>
                                <ChevronRight size={20} />
                            </div>
                            ...
                            <Swiper 
                                ...
                                loop={true}
                                navigation={{
                                    nextEl: `.swiper-button-next-${similarHouse.id}`,
                                    prevEl: `.swiper-button-prev-${similarHouse.id}`,
                                }}
                                pagination={{
                                    // el: `.swiper-pagination-${listing.id}`,
                                    clickable: true,
                            ...
                            >
                                {similarHouse.images.map((img, index) => (
                                    <SwiperSlide key={index}>
                                    <img
                                        src={img}
                            ...
                            <div className="absolute bottom-2 right-2 flex items-center gap-1 bg-black/70 text-white px-2 py-1 rounded-md text-sm">
                                <Camera size={14} />
                                <span>{similarHouse.images.length || similarHouse.imageCount}</span>
                            </div>

                            {/* Content */}
                            <div className="p-4">
                                <p className="text-gray-600 text-sm">KSH <span className="text-xl font-bold">{similarHouse.price}</span></p>
                            
                                <div className="flex items-center justify-between text-gray-700 mt-3 text-sm space-x-4">
                                    <span className="flex items-center"><Bed className="w-4 h-4 mr-1" /> {similarHouse.type}</span>
                                    <span>• {similarHouse.furnished}</span>
                                </div>

                                <div className="flex items-center justify-between text-gray-700 mt-3 text-sm">
                                    <span className="flex items-center"><MapPin className="w-4 h-4 mr-1" />{similarHouse.location}</span>
                                    <span className="ml-2"> {similarHouse.category}</span>
                                </div>

                                <p className="text-gray-600 mt-3 text-sm line-clamp-2">
                                    {similarHouse.description}
                                </p>

                                <button 
                                    className="text-blue-600 mt-3 text-sm font-medium hover:underline hover:cursor-pointer"
                                    // onClick={() => handleClick(similarHouse)}
                                >
                                    Read More
                                </button>

                                <div className="mt-4 flex items-center">
                                    <span className="text-gray-800 text-sm font-medium">{similarHouse.agent}</span>
                                </div>
                            </div>
                        </div>
                    ))}
