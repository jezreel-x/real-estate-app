...
const HouseForRentDetails = () => {
    ...
    useEffect(() => {
        // Retrieve the selected house listing from local storage
        const listing = localStorage.getItem("houseRentals");
        if (listing) {
            setHouse(JSON.parse(listing));

            // Filter out the selected listing from all listings to get similar houses
            const allListings = localStorage.getItem("allHouseRentals");
            if (allListings) {
                const listingsArray = JSON.parse(allListings);
                const similar = listingsArray
                    .filter(item => item.id !== JSON.parse(listing).id)
                    .sort(() => 0.5 - Math.random()) // Shuffle the array
                    .slice(0, 5) // Get first 3 items
    ...
    return (
        <div className="flex flex-col w-full mx-auto min-h-screen">
            ...
            <div className="flex mt-24 mb-4 p-6 items-center">
                ...
                <Link to="/houses-for-rent" className="text-gray-900 font-semibold hover:text-[rgb(0,0,122)] flex items-center justify-center text-lg sm:text-xl">
                    Houses For Rent
                </Link>
            </div>
            ...
            <div className="mt-8">
                <h2 className="text-2xl font-semibold text-gray-800 mb-4 px-6">Similar House Rentals</h2>
                <div className="grid lg:grid-cols-2 xl:grid-cols-3 3xl:grid-cols-4 gap-6 px-6">
                    {similarRentals.map((similarRental) => (
                        ...
                        >
                            {/* Custom Navigation Buttons */}
                            <div className={`absolute top-1/4 left-2 z-10 -translate-y-1/2 cursor-pointer swiper-button-prev-${similarRental.id} bg-black/50 p-2 rounded-full text-white hover:bg-black/70`}>
                                <ChevronLeft size={20} />
                            </div>
                            <div className={`absolute top-1/4 right-2 z-10 -translate-y-1/2 cursor-pointer swiper-button-next-${similarRental.id} bg-black/50 p-2 rounded-full text-white hover:bg-black/70`}>
                                <ChevronRight size={20} />
                            </div>
                            ...
                            <Swiper 
                                ...
                                loop={true}
                                navigation={{
                                    nextEl: `.swiper-button-next-${similarRental.id}`,
                                    prevEl: `.swiper-button-prev-${similarRental.id}`,
                                }}
                                pagination={{
                                    // el: `.swiper-pagination-${listing.id}`,
                                    clickable: true,
                            ...
                            >
                                {similarRental.images.map((img, index) => (
                                    <SwiperSlide key={index}>
                                    <img
                                        src={img}
                                        alt={similarRental.title}
                            ...
                            <div className="absolute bottom-2 right-2 flex items-center gap-1 bg-black/70 text-white px-2 py-1 rounded-md text-sm">
                                <Camera size={14} />
                                <span>{similarRental.images.length || similarRental.imageCount}</span>
                            </div>

                            {/* Content */}
                            <div className="p-4">
                                <p className="text-gray-600 text-sm">KSH <span className="text-xl font-bold">{similarRental.price}</span></p>
                            
                                <div className="flex items-center justify-between text-gray-700 mt-3 text-sm space-x-4">
                                    <span className="flex items-center"><Bed className="w-4 h-4 mr-1" /> {similarRental.type}</span>
                                    <span>• {similarRental.furnished}</span>
                                </div>

                                <div className="flex items-center justify-between text-gray-700 mt-3 text-sm">
                                    <span className="flex items-center"><MapPin className="w-4 h-4 mr-1" />{similarRental.location}</span>
                                    <span className="ml-2"> {similarRental.category}</span>
                                </div>

                                <p className="text-gray-600 mt-3 text-sm line-clamp-2">
                                    {similarRental.description}
                                </p>

                                <button 
                                    className="text-blue-600 mt-3 text-sm font-medium hover:underline hover:cursor-pointer"
                                    // onClick={() => handleClick(similarRental)}
                                >
                                    Read More
                                </button>

                                <div className="mt-4 flex items-center">
                                    <span className="text-gray-800 text-sm font-medium">{similarRental.agent}</span>
                                </div>
                            </div>
                        </div>
                    ))}
